...
class TBLogger:
  ...
  def __init__(self, enabled, log_dir, name, interval=1, dummies=True):
    ...
    self.cache = {}
    if self.enabled:
      self.summary_writer = SummaryWriter(log_dir=Path(log_dir, name),
                                          flush_secs=120,
                                          max_queue=200)
...
def init(log_fpath, log_dir, enabled=True, tb_subsets=[], **tb_kw):
  ...
  global tb_loggers
  tb_loggers = {
      s: TBLogger(enabled, log_dir, name=s, **tb_kw) for s in tb_subsets
  }
...
def init_inference_metadata(batch_size=None):
  ...
  modalities = [('latency', 's', ':>10.5f'), ('RTF', 'x', ':>10.2f'),
                ('frames/s', 'frames/s', ':>10.2f'),
                ('samples/s', 'samples/s', ':>10.2f'),
                ('letters/s', 'letters/s', ':>10.2f'),
                ('tokens/s', 'tokens/s', ':>10.2f')]
...
def parameters(data, verbosity=0, tb_subset=None):
  ...
  if tb_subset is not None and tb_loggers[tb_subset].enabled:
    tb_data = {
        k: v for k, v in data.items() if type(v) in (str, bool, int, float)
    }
