...
hardcoded_acronyms = [
    'BMW', 'MVD', 'WDSU', 'GOP', 'UK', 'AI', 'GPS', 'BP', 'FBI', 'HD', 'CES',
    'LRA', 'PC', 'NBA', 'BBL', 'OS', 'IRS', 'SAC', 'UV', 'CEO', 'TV', 'CNN',
    'MSS', 'GSA', 'USSR', 'DNA', 'PRS', 'TSA', 'US', 'GPU', 'USA', 'FPCC', 'CIA'
]
...
_non_uppercase_re = re.compile(
    r'\b({})\b'.format('|'.join(non_uppercase_exceptions.keys())),
    re.IGNORECASE)
...
def spell_acronyms(text):
  text = re.sub(_non_uppercase_re,
                lambda m: non_uppercase_exceptions[m.group(0).lower()], text)
