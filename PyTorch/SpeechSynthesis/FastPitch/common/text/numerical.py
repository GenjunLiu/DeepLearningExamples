...
_magnitudes = [
    'trillion', 'billion', 'million', 'thousand', 'hundred', 'm', 'b', 't'
]
...
_currency_re = re.compile(
    r'([\$€£₩])([0-9\.\,]*[0-9]+)(?:[ ]?({})(?=[^a-zA-Z]|$))?'.format(
        "|".join(_magnitudes)), re.IGNORECASE)
...
def _expand_currency(m):
  ...
  if magnitude is not None and magnitude.lower() in _magnitudes:
    ...
    return "{} {} {}".format(_expand_hundreds(quantity), magnitude,
                             currency + 's')
...
def _expand_number(m):
  ...
  if number > 1000 < 10000 and (number % 100 == 0) and (number % 1000 != 0):
    ...
    if number == 2000:
      ...
      number = _inflect.number_to_words(number, andword='', zero='oh',
                                        group=2).replace(', ', ' ')
