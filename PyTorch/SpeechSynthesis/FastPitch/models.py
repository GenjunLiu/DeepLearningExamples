...
def convert_ts_to_trt(model_name, ts_model, parser, amp, unk_args=[]):
  ...
  if model_name == 'HiFi-GAN':
    return _convert_ts_to_trt_hifigan(ts_model, amp,
                                      trt_args.trt_min_opt_max_batch,
...
def _parse_trt_compilation_args(model_name, parent, add_help=False):
  ...
  parser = argparse.ArgumentParser(parents=[parent],
                                   add_help=add_help,
                                   allow_abbrev=False)
  trt = parser.add_argument_group(
      f'{model_name} Torch-TensorRT compilation parameters')
  trt.add_argument('--trt-min-opt-max-batch',
                   nargs=3,
                   type=int,
                   default=(1, 8, 16),
  ...
  if model_name == 'HiFi-GAN':
    trt.add_argument(
        '--trt-min-opt-max-hifigan-length',
        nargs=3,
        type=int,
...
def _convert_ts_to_trt_hifigan(ts_model,
                               amp,
                               trt_min_opt_max_batch,
  ...
  min_shp, opt_shp, max_shp = zip(trt_min_opt_max_batch, (num_mels,) * 3,
                                  trt_min_opt_max_hifigan_length)
  compile_settings = {
      "inputs": [
