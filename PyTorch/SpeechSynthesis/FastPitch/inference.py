...
def parse_args(parser):
  ...
  parser.add_argument('--report-mel-loss',
                      action='store_true',
...
def main():
  ...
  if args.affinity != 'disabled':
    ...
    affinity = gpu_affinity.set_affinity(0, nproc_per_node, args.affinity)
  ...
  def _load_pyt_or_ts_model(model_name, ckpt_path):
    if args.checkpoint_format == 'ts':
      model = models.load_and_setup_ts_model(model_name, ckpt_path, args.amp,
                                             device)
      model_train_setup = {}
  ...
  if args.waveglow is not None:
    ...
    voc_name = 'hifigan'
    vocoder, voc_train_setup = _load_pyt_or_ts_model('HiFi-GAN', args.hifigan)

    if args.denoising_strength > 0.0:
      denoiser = Denoiser(vocoder, win_length=args.win_length).to(device)

    if args.torch_tensorrt:
      vocoder = models.convert_ts_to_trt('HiFi-GAN', vocoder, parser, args.amp,
                                         unk_args)
  ...
  batches = prepare_input_sequence(fields,
                                   ...
                                   args.batch_size,
                                   args.dataset_path,
                                   load_mels=(generator is None or
                                              args.report_mel_loss),
  ...
  for rep in (tqdm(range(reps), 'Inference') if reps > 1 else range(reps)):
    for b in batches:
      ...
      if generator is None:
        ...
        if args.report_mel_loss:
          gen_mel_loss_sum += compute_mel_loss(mel, mel_lens, b['mel'],
                                               b['mel_lens'])
      ...
      if vocoder is not None:
        ...
        vocoder_infer_perf = (audios.size(0) * audios.size(1) /
                              vocoder_measures[-1])
        ...
        if args.output is not None and reps == 1:
          for i, audio in enumerate(audios):
            ...
            fname = b['output'][
                i] if 'output' in b else f'audio_{all_utterances + i}.wav'
  ...
  if generator is not None:
    ...
    log(
        (), {
            f"90%_{gen_name}_latency": gm.mean() + norm.ppf(
                (1.0 + 0.90) / 2) * gm.std()
        })
    log(
        (), {
            f"95%_{gen_name}_latency": gm.mean() + norm.ppf(
                (1.0 + 0.95) / 2) * gm.std()
        })
    log(
        (), {
            f"99%_{gen_name}_latency": gm.mean() + norm.ppf(
                (1.0 + 0.99) / 2) * gm.std()
        })
    if args.report_mel_loss:
  ...
  if vocoder is not None:
    ...
    log(
        (), {
            f"90%_{voc_name}_latency": vm.mean() + norm.ppf(
                (1.0 + 0.90) / 2) * vm.std()
        })
    log(
        (), {
            f"95%_{voc_name}_latency": vm.mean() + norm.ppf(
                (1.0 + 0.95) / 2) * vm.std()
        })
    log(
        (), {
            f"99%_{voc_name}_latency": vm.mean() + norm.ppf(
                (1.0 + 0.99) / 2) * vm.std()
        })
    if args.report_mel_loss:
