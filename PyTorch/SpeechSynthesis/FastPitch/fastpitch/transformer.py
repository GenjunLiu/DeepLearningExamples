...
class MultiHeadAttn(nn.Module):
  ...
  def _forward(self, inp, attn_mask=None):
    ...
    attn_vec = attn_vec.permute(1, 2, 0,
                                3).contiguous().view(inp.size(0), inp.size(1),
                                                     n_head * d_head)
...
class TransformerLayer(nn.Module):
  ...
  def __init__(self, n_head, d_model, d_head, d_inner, kernel_size, dropout,
    ...
    self.pos_ff = PositionwiseConvFF(d_model,
                                     d_inner,
                                     kernel_size,
                                     dropout,
...
class FFTransformer(nn.Module):
  ...
  def __init__(self,
    ...
    if embed_input:
      self.word_emb = nn.Embedding(n_embed,
                                   d_embed or d_model,
